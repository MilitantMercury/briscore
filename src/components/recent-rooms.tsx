...
export function RecentRooms({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  return (
    <details className="recent-menu">
      <summary>Le mie partite</summary>
      <div className="recent-menu-list">
        <span className="eyebrow">LE TUE PARTITE</span>
        {error && <p role="status">{error}</p>}
        {rooms.map((room) => (
          <button key={room.id} className="secondary" onClick={() => onOpen(room.id)}>
            Partita del {new Date(room.created_at).toLocaleString("it-IT", {
