...
export default function MyGamesPage() {
  ...
  async function search(event: React.FormEvent) { event.preventDefault(); setSearchError(""); setSearching(true); try { const response = await fetch(`/api/rooms/search?code=${encodeURIComponent(code)}`, { cache: "no-store" }); const result = await response.json(); if (!response.ok) throw new RequestError(result.error || "Ricerca non disponibile.", response.status, result.code || ""); router.push(`/?room=${result.id}&code=${result.publicCode}`); } catch (e) { setSearchError(e instanceof RequestError && e.status === 404 ? "Nessuna partita trovata con questo codice." : "Ricerca non disponibile. Riprova tra poco."); } finally { setSearching(false); } }
