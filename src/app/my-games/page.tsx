...
export default function MyGamesPage() {
  ...
  return <main className="standalone-page games-scene">
    ...
    <section className="games-hero-card">
      ...
      {!loading && !error && <span className="games-counter">{games.length} {games.length === 1 ? "partita" : "partite"}</span>}
    </section>
    ...
    {!loading && !error && !games.length && <section className="empty-deck"><span>DENARI · COPPE · SPADE · BASTONI</span><h2>Il tuo mazzo è ancora vuoto.</h2><p>Crea una partita e qui ritroverai tutti i tavoli.</p><Link className="primary" href="/">Crea una partita →</Link></section>}
    <div className="games-deck">
      {games.map((g,index)=>{
        ...
        return <Link className={`game-card game-${status}`} key={g.id} href={`/?room=${g.id}`}>
          ...
          <div className="game-card-main"><span className={`game-status status-${status}`}>{label}</span><h2>Partita del {new Date(g.created_at).toLocaleDateString("it-IT",{day:"numeric",month:"long"})}</h2><p>Ultimo movimento {new Date(g.updated_at).toLocaleString("it-IT",{dateStyle:"short",timeStyle:"short"})}</p></div>
