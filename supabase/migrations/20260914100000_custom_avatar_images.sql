...
create policy "authenticated users see avatars" on storage.objects for select to authenticated using (bucket_id = 'avatars');
...
language sql stable security definer set search_path = '' as $$
  select jsonb_strip_nulls(jsonb_build_object(
    ...
    'members',(select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object('userId',m.user_id,'playerId',m.player_id,'avatar',coalesce(nullif(u.raw_user_meta_data->>'avatar',''),'bastoni'),'avatarImage',nullif(u.raw_user_meta_data->>'avatarImage',''),'globalRank',case when briscore_private.global_rank(m.user_id)<=3 then briscore_private.global_rank(m.user_id) else null end)) order by p.seat nulls last),'[]'::jsonb) from public.room_members m join auth.users u on u.id=m.user_id left join public.players p on p.id=m.player_id where m.room_id=r.id),
